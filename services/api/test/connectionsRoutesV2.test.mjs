...
async function buildRouteApp({
  permission = { ok: true, session: session() },
  ...
  app.decorate("requirePermission", async (request, permissions, options) => {
    if (typeof permissionResolver === "function") {
      return permissionResolver(request, permissions, options);
    }
    return permission;
  });
...
test("connection routes expose only dedicated bounded permission constants", () => {
  assert.deepEqual(READ_PERMISSIONS, ["OWNER_ADMIN_CONNECTION_READ"]);
  assert.deepEqual(PROFILE_WRITE_PERMISSIONS, ["OWNER_ADMIN_CONNECTION_WRITE"]);
  assert.deepEqual(SECURITY_WRITE_PERMISSIONS, ["OWNER_ADMIN_CONNECTION_SECRET_MANAGE"]);
  assert.deepEqual(TEST_PERMISSIONS, ["OWNER_ADMIN_CONNECTION_TEST"]);
});

test("read, write, secret, and test endpoints request their dedicated permissions", async (t) => {
  const requested = [];
  ...
  const cases = [
    {
      ...
      expected: ["OWNER_ADMIN_CONNECTION_READ"],
    },
    {
      method: "POST",
      url: "/owner-admin/connections",
      payload: { identity: { connection_code: "safe_code", connection_name: "Safe" } },
      expected: ["OWNER_ADMIN_CONNECTION_WRITE"],
    },
    {
      method: "GET",
      url: "/owner-admin/connections/safe_code/secrets",
      expected: ["OWNER_ADMIN_CONNECTION_SECRET_MANAGE"],
    },
    {
      method: "POST",
      url: "/owner-admin/connections/safe_code/test",
      expected: ["OWNER_ADMIN_CONNECTION_TEST"],
    },
  ];
  ...
  for (const request of cases) {
    ...
    assert.deepEqual(response.json().required_permissions, request.expected);
  }

  assert.deepEqual(requested, cases.map((entry) => entry.expected));
});
...
test("connections fail closed when owner-admin connection permission is absent", async (t) => {
  const app = await buildRouteApp({
    permission: {
      ok: false,
      status: 403,
  ...
  assert.deepEqual(response.json().required_permissions, ["OWNER_ADMIN_CONNECTION_READ"]);
});
